...
if [[ "$#" -lt 2 ]]; then
  echo "Usage: ./map.sh ref input.fofn map_opt [i]"
  echo "Map reads with minimap2."
...
set -x
module load minimap2/2.30 # load v2.30+
module load samtools/1.23 # load v1.15.1+

minimap2 --MD -a $map_opt -I12g $opt -t$cpus $ref $reads > $tmp/$out.sam
